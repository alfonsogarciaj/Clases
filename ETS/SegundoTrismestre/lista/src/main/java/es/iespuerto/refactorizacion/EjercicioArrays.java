...
public class EjercicioArrays {
    ...
    public static void main(String[] args) {
        ...
        int numAlumnos  = sc.nextInt();sc.nextLine();
        ...
        control = generarNotasAletorias(control);
        //buscamos al mayor
        maxNota = buscarNotaMayor(control);
        //buscamos al menor
        minNota = buscarNotaMenor(control);
        //creamos una lista de los alumnos de la clase
        int[] listaClase = new int[numAlumnos];
        listaClase = listaClase(listaClase, numAlumnos);
        ...
        practicas = practicas(practicas, numAlumnos);
    ...
    public static Integer[] generarNotasAletorias(Integer[] control ){
        for(int i=0; i < control.length; i++){
    ...
    public static int buscarNotaMenor(Integer[] control){
        int postEval = 11;
        int minNota = 0;
        for(int i=0; i<control.length; i++){
    ...
    public static int buscarNotaMayor(Integer[] control){
        int postEval = 0;
        int maxNota = 0;
        for(int i=0; i<control.length; i++){
            int preEval = control[i];
    ...
    public static int[] listaClase(int[] lista, int numAlumnos){
        int[] listaClase = new int[numAlumnos];
        for (int i = 0; i < numAlumnos; i++){
            listaClase[i] = i+1;
        }
    ...
    public static int[] practicas(int[] pract, int numAlumnos){
        int[] practicas = new int[numAlumnos];
